...
(0, _pixabayApi.fetchImages)("cat").then((data)=>console.log(data)).catch((error)=>console.error("Error:", error));
// Variabile globale
let currentPage = 1;
let currentQuery = "";
let totalHits = 0;
...
form.addEventListener("submit", async (event)=>{
    ...
    try {
        const { hits , totalHits: hitsCount  } = await (0, _pixabayApi.fetchImages)(currentQuery, currentPage);
        totalHits = hitsCount;
        if (hits.length === 0) {
            (0, _notiflixDefault.default).Notify.failure("Sorry, there are no images matching your search query.");
            return;
        }
        (0, _notiflixDefault.default).Notify.success(`Hooray! We found ${totalHits} images.`);
        renderGallery(hits);
        if (totalHits > hits.length) loadMoreButton.style.display = "block";
    } catch (error) {
...
loadMoreButton.addEventListener("click", async ()=>{
    currentPage += 1;
    try {
        const { hits  } = await (0, _pixabayApi.fetchImages)(currentQuery, currentPage);
        renderGallery(hits);
...
},{"./pixabay-api":"ebejz","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","notiflix":"fuMq1","regenerator-runtime/runtime":"dXNgZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"ebejz":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
